...
export class PersonRepository {
  ...
  public async create(input: CreatePersonInput): Promise<Person> {
    const result = await this.database.query<PersonRow>(
      `INSERT INTO persons (external_id, full_name, email, organization_id, department_id, position_name) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id, external_id, full_name, email, organization_id, department_id, position_name, is_active`,
      [
  ...
  public async findById(personId: string): Promise<Person | null> {
    const result = await this.database.query<PersonRow>(
      'SELECT id, external_id, full_name, email, organization_id, department_id, position_name, is_active FROM persons WHERE id = $1',
      [personId]
    );
  ...
  public async findAll(): Promise<Person[]> {
    const result = await this.database.query<PersonRow>(
      'SELECT id, external_id, full_name, email, organization_id, department_id, position_name, is_active FROM persons ORDER BY full_name, id'
    );
